...
ScriptExtension::ScriptExtension(QObject* parent) : QObject(parent) {}
...
QString ScriptExtension::httpRequest(const QString& url, const QVariantMap& headers)
{
    ...
    VERIFY(connect(reply, SIGNAL(finished()), SLOT(onHttpRequestFinished())));
...
void ScriptExtension::clearCookies(const QString& domain)
{
    QList<QNetworkCookie> cookies =
        static_cast<CookieJarPublicMorozov*>(TheQNetworkAccessManager::Instance().cookieJar())->allCookies();
    cookies.erase(
