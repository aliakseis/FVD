...
bool ParseThread::readFrame(AVPacket* packet)
{
	...
	if (parent->m_bytesLimiter >= 0 && parent->m_bytesLimiter - parent->m_bytesCurrent < PLAYBACK_AVPACKET_MAX)
	{
		...
		parent->m_videoPacketsQueue.enqueue(parent->m_downloadingPacket, &parent->m_packetsQueueMutex, &parent->m_packetsQueueCV);
		parent->m_audioPacketsQueue.enqueue(parent->m_downloadingPacket, &parent->m_packetsQueueMutex, &parent->m_packetsQueueCV);

		double longSleepTime = parent->m_waitSleperTime;
		while ((parent->m_bytesLimiter >= 0 && parent->m_bytesLimiter - parent->m_bytesCurrent < PLAYBACK_AVPACKET_MAX) || (parent->m_bytesLimiter >= 0 && longSleepTime > 0 && m_seekDuration < 0))   // 512kb move
		{
			TAG("ffmpeg_readpacket_limitation") << "Wait downloading limitation = " << PLAYBACK_AVPACKET_MAX - (parent->m_bytesLimiter - parent->m_bytesCurrent);
			...
			if (packet.stream_index == parent->m_videoStreamNumber)
			{
					...
					[this, parent]()
				{
					return parent->m_audioPacketsQueue.empty()
						   || parent->m_videoPacketsQueue.packetsSize() < MAX_QUEUE_SIZE && parent->m_videoPacketsQueue.size() < MAX_VIDEO_FRAMES
						   || m_seekDuration >= 0;
				},
			...
			else if (packet.stream_index == parent->m_audioStreamNumber)
			{
					...
					[this, parent]()
				{
					return parent->m_videoPacketsQueue.empty()
						   || parent->m_audioPacketsQueue.packetsSize() < MAX_QUEUE_SIZE && parent->m_audioPacketsQueue.size() < MAX_AUDIO_FRAMES
						   || m_seekDuration >= 0;
				},
				&parent->m_packetsQueueMutex);
...
void ParseThread::sendSeekPacket()
{
	...
	if (m_seekDuration >= 0 && (
				(((parent->m_seekFlags & 0x10) != 0) && ((parent->m_seekFlags & 0x20) != 0))
				||  parent->m_seekFlags == 0x00)) // seeking flags reseted
	{
		TAG("ffmpeg_seek") << "Begin seek";

		if (avformat_seek_file(parent->m_formatContext, parent->m_videoStreamNumber, 0, m_seekDuration, m_seekDuration, AVSEEK_FLAG_FRAME) < 0)
		{
