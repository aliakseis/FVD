...
void FFmpegDecoder::close(bool isBlocking)
{
	...
	if (
		current_thread == m_mainVideoThread
		||	current_thread == m_mainAudioThread
		||	current_thread == m_mainParseThread
		||	current_thread == m_mainDisplayThread
...
void FFmpegDecoder::setVolume(int volume, int steep)
{
	setVolume((double)volume / steep);
}

double FFmpegDecoder::volume() const
{
    return m_volume;
}
...
FPicture* FFmpegDecoder::frameToImage()
{
	...
	m_imageCovertContext = sws_getCachedContext(
	...
	VERIFY(sws_scale(
			   ...
			   0,
			   m_videoFrame->height,
			   m_videoFrameData.data,
			   m_videoFrameData.linesize
		   ) > 0);
...
double FFmpegDecoder::videoClock() const
{
	return m_videoPTS;
}

double FFmpegDecoder::audioClock() const
{
	return m_audioPTS;
}
...
{
	int64_t DesiredFrameNumber = av_rescale(tsms, m_formatContext->streams[m_videoStreamNumber]->time_base.den, m_formatContext->streams[m_videoStreamNumber]->time_base.num);
...
bool FFmpegDecoder::seekFrame(int64_t frame)
{
	return seekDuration(((double)frame / m_frameTotalCount) * m_duration);
}

bool FFmpegDecoder::seekDuration(int64_t duration)
{
	if (m_bytesLimiter >= 0 && ((m_durationLimiter >= 0 && duration > m_durationLimiter) ||
	...
	if ((m_mainParseThread != nullptr) && m_mainParseThread->m_seekDuration == -1
			&& m_mainAudioThread != nullptr && !m_mainAudioThread->m_isSeekingWhilePaused
			&& m_mainVideoThread != nullptr && !m_mainVideoThread->m_isSeekingWhilePaused)
	{
...
int64_t FFmpegDecoder::duration()const
{
	return m_duration;
}
        ...
        for (const auto percent : percents)
        {
            ...
            if (av_seek_frame(m_formatContext, m_videoStreamNumber, frame, 0) >= 0)
            {
                for (;;)
                {
                    ...
                    {
                        if (0 == frame || (avcodec_flush_buffers(m_videoCodecContext), av_seek_frame(m_formatContext, m_videoStreamNumber, 0, 0) < 0))
                        {
                            break;
                        }
                        frame = 0;
...
int64_t FFmpegDecoder::framesCount()const
{
	return m_frameTotalCount;
}

int64_t FFmpegDecoder::headerSize() const
{
    return FFMAX(0, m_headerSize - 8);
}
...
bool FFmpegDecoder::isBrokenDuration() const
{
	return m_fileProbablyNotFull;
}

void FFmpegDecoder::setPixelFormat(AVPixelFormat format)
{
	m_pixelFormat = format;
}

void FFmpegDecoder::setResizeWithDecoder(bool policy)
{
	m_resizeWithDecoder = policy;
}
