...
/// \class DownloaderObserverInterface
/// \brief Observer interface for the Downloader.
struct DownloaderObserverInterface
{
    virtual void onFinished() = 0;
    virtual void onError(utilities::ErrorCode::ERROR_CODES code,
    ...
    virtual ~DownloaderObserverInterface() = default;
};
...
struct IDownloader
{
    ...
    virtual void setDownloadNamePolicy(
        DuplicateDownloadNamePolicy download_name_policy) = 0;

    virtual void Start(
        const QList<QUrl>& urls,
        QNetworkAccessManager* network_manager,
        const QString& filename = QString(),
        const QStringList& httpHeaders = QStringList()) = 0;
    ...
    virtual void setObserver(
        DownloaderObserverInterface* observer) = 0;
};
