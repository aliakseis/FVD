...
SitesCombo::SitesCombo(QWidget* parent) : QComboBox(parent), m_containerMousePress(false)
{
    VERIFY(connect(this, SIGNAL(activated(int)), SLOT(toggleCheckState(int))));
...
void SitesCombo::showPopup() { QComboBox::showPopup(); }
...
QStringList SitesCombo::selectedSites() const { return m_selectedStrategies; }
...
void SitesCombo::fitContent()
{
    ...
    {
        QString cutText = fontMetrics().elidedText(itemData(i, TextRole).toString(), Qt::ElideRight,
                                                   width() - 64);  // 64 is just margin for two icons and text margins
