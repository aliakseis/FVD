      ...
      isUpdatingState(false)
{
    ...
    VERIFY(connect(&timer, SIGNAL(timeout()), SLOT(onTimeout())));

    utilities::Tr::MakeRetranslatable(this, ui);
}

DownloadsControl::~DownloadsControl() { delete ui; }

void DownloadsControl::on_btnStart_clicked() { emit start(); }

void DownloadsControl::on_btnPause_clicked() { emit pause(); }

void DownloadsControl::on_btnStop_clicked() { emit stop(); }

void DownloadsControl::on_btnRemove_clicked()
{
    if (QMessageBox::question(
            nullptr, utilities::Tr::Tr(PROJECT_FULLNAME_TRANSLATION),
            tr("Are you sure you want to remove this download? If download completed file will not be removed."),
            QMessageBox::Yes, QMessageBox::No) == QDialogButtonBox::Yes)
    {
        emit remove();
    }
}

void DownloadsControl::on_btnRestart_clicked() { emit reload(); }

void DownloadsControl::on_btnUp_clicked() { emit up(); }
...
void DownloadsControl::focusInEvent(QFocusEvent* event) { QWidget::focusInEvent(event); }
...
void DownloadsControl::setState(State state, bool isUpEnabled, bool isDownEnabled, bool isStopEnabled,
                                bool isStartEnabled, int speedLimit)
{
...
void DownloadsControl::show() { QWidget::show(); }

void DownloadsControl::hide() { QWidget::hide(); }
