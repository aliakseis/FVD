    ...
    if (element == QStyle::CE_ProgressBarContents)
    {
        ...
        if (const auto* bar = qstyleoption_cast<const QStyleOptionProgressBar*>(option))
        {
            ...
            int progressBarWidth =
                (progress - bar->minimum) * qreal(maxWidth) / qMax(qreal(1.0), qreal(bar->maximum) - bar->minimum);
...
void FaenzaStyle::drawPrimitive(PrimitiveElement element, const QStyleOption* option, QPainter* painter,
                                const QWidget* widget) const
{
