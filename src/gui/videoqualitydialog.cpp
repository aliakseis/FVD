	...
	m_entity(entity)
{
	...
	else
	{
		...
		entity->extractLinks();
		VERIFY(connect(entity, SIGNAL(linksExtracted()), this, SLOT(onLinksExtracted())));
	}
	VERIFY(connect(ui->listWidget, SIGNAL(itemClicked(QListWidgetItem*)), this, SLOT(onItemClicked(QListWidgetItem*))));
}
...
{
	if ((static_cast<int>(event->type() == QEvent::WindowDeactivate) | static_cast<int>(event->type() == QEvent::HideToParent)) != 0)
	{
		close();
		localEventLoop.quit();
	}
