...
DownloadsForm::DownloadsForm(VideoPlayerWidget* video_widget, QWidget* parent) :
	QFrame(parent),
	ui(new Ui::DownloadsForm),
	videoPlayer(video_widget),
	control(nullptr)
{
	ui->setupUi(this);
	...
	VERIFY(connect(ui->leSearch, SIGNAL(returnPressed()), this, SLOT(onSearch())));
	VERIFY(connect(ui->btnSearch, SIGNAL(clicked()), this, SLOT(onSearch())));
	VERIFY(connect(ui->downloadsTreeView, SIGNAL(customContextMenuRequested(QPoint)), this, SLOT(onDownloadsContextMenu(QPoint))));
	VERIFY(connect(ui->downloadsTreeView, SIGNAL(doubleClicked(QModelIndex)), this, SLOT(onDownloadsDoubleClick(QModelIndex))));
	VERIFY(connect(ui->downloadsTreeView->selectionModel(), SIGNAL(selectionChanged(QItemSelection, QItemSelection)), this, SLOT(onSelectionChanged(QItemSelection, QItemSelection))));
	VERIFY(connect(ui->btnClear, SIGNAL(clicked()), this, SLOT(clearDownloadsList())));
	VERIFY(connect(control, SIGNAL(up()), this, SLOT(onItemUp())));
	VERIFY(connect(control, SIGNAL(down()), this, SLOT(onItemDown())));
	VERIFY(connect(control, SIGNAL(start()), this, SLOT(onStartItem())));
	VERIFY(connect(control, SIGNAL(stop()), this, SLOT(onStopItem())));
	VERIFY(connect(control, SIGNAL(reload()), this, SLOT(onReloadItem())));
	VERIFY(connect(control, SIGNAL(remove()), this, SLOT(onRemoveItem())));
	VERIFY(connect(control, SIGNAL(pause()), this, SLOT(onPauseItem())));
	VERIFY(connect(control, SIGNAL(setSpeedLimit(int)), this, SLOT(onSetSpeedLimit(int))));

	VERIFY(connect(ui->downloadsTreeView->verticalScrollBar(), SIGNAL(valueChanged(int)), this, SLOT(onVerticalScrollChanged(int))));
...
void DownloadsForm::onDownloadsContextMenu(const QPoint& point)
{
	...
	if (selection)
	{
		...
		{
			deleteItems(ui->downloadsTreeView->selectionModel()->selectedRows(), qApp->keyboardModifiers() & Qt::ShiftModifier);
		}
	}
	else
	{
...
void DownloadsForm::onReloadItem()
{
	...
	{
		VERIFY(connect(videoPlayer, SIGNAL(fileReleased()), this, SLOT(restartDownload()), Qt::UniqueConnection));
...
void DownloadsForm::onRemoveItem()
{
	...
	if (fileIsInUse(videoPlayer, m_model->item(mappedRow)))
	{
		VERIFY(connect(videoPlayer, SIGNAL(fileReleased()), this, SLOT(removeDownload()), Qt::UniqueConnection));
...
void DownloadsForm::onActivated(const DownloadEntity* selEntity)
{
	...
	if (selEntity != nullptr)
	{
		...
		selection << QItemSelectionRange(m_proxyModel->index(proxyIndex, 0), m_proxyModel->index(proxyIndex, DL_LastColumn - 1));
...
void DownloadsForm::onPreferencesChanged()
{
	m_model->considerStartNextDownload();
}
...
void DownloadsForm::deleteItems(QModelIndexList const& indexList, bool deleteCompletely)
{
	...
	QString warningText = (deleteCompletely ?
	...
	if (QMessageBox::Yes == QMessageBox::question(this, tr("Warning"), warningText, QMessageBox::Yes, QMessageBox::No))
	{
		...
		std::transform(
			indexList.constBegin(), indexList.constEnd(),
			std::back_inserter(itemsToDelete),
			[this](const QModelIndex & index)->DownloadEntity* {return m_model->item(m_proxyModel->mapToSource(index).row());}
		);
		// find currently played entity in candidates to delete
		auto played = std::find_if(
						  itemsToDelete.constBegin(), itemsToDelete.constEnd(),
						  [capture0 = VideoPlayerWidgetInstance()](auto && PH1) { return fileIsInUse(capture0, std::forward<decltype(PH1)>(PH1)); }
					  );
	...
	if (obj == ui->downloadsTreeView->viewport())
	{
		   ...
		   )
		{
			...
			if (index.isValid() && index.column() > 1)
			{
				...
				control->setState((entity->state() == DownloadEntity::kDownloading) ? DownloadsControl::Started : DownloadsControl::Paused,
								  (proxyRow > 0), (proxyRow < (m_proxyModel->rowCount() - 1)) , DownloadListModel::canStopDownload(entity), is_edownload,
								  entity->speedLimit());
...
QWidget* DownloadsForm::manageWidget() const
{
	return ui->manageLabel;
}

void DownloadsForm::clearDownloadsList(bool silent/* =false*/)
{
	if (m_model->rowCount() > 0
			&& (silent || QMessageBox::question(nullptr, Tr::Tr(PROJECT_FULLNAME_TRANSLATION), tr("<b>Are you sure want to clear the download list?</b>"), QMessageBox::Yes | QMessageBox::No) == QMessageBox::Yes))
	{
		m_model->clear();
		emit entityActivated(nullptr);
	}
...
void DownloadsForm::onSearch()
{
	m_proxyModel->setFilterFixedString(ui->leSearch->text());
}
...
void DownloadsForm::moveUpSelectedItems()
{
	m_proxyModel->moveItemsUp(ui->downloadsTreeView->selectionModel());
}

void DownloadsForm::moveDownSelectedItems()
{
	m_proxyModel->moveItemsDown(ui->downloadsTreeView->selectionModel());
}
...
void DownloadsForm::onVerticalScrollChanged(int /*unused*/)
{
	hideFloatingControl();
}
