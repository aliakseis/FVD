	...
	m_isModelFileOutdated(false)
{
	...
	VERIFY(connect(m_mainToolbar, SIGNAL(settings()), this, SLOT(openPreferences())));
	VERIFY(connect(m_mainToolbar, SIGNAL(search()), this, SLOT(activateSearchForm())));
	VERIFY(connect(m_mainToolbar, SIGNAL(downloads(const DownloadEntity*)), this, SLOT(activateDownloadsForm(const DownloadEntity*))));
	VERIFY(connect(m_mainToolbar, SIGNAL(library(const DownloadEntity*)), this, SLOT(activateLibraryForm(const DownloadEntity*))));
	VERIFY(connect(m_mainToolbar, SIGNAL(help()), this, SLOT(about())));
	VERIFY(connect(m_searchResultFrom, SIGNAL(entityActivated(RemoteVideoEntity*)), this, SLOT(onSearchItemActivated(RemoteVideoEntity*))));
	VERIFY(connect(m_downloadsForm, SIGNAL(entityActivated(const DownloadEntity*)), this, SLOT(onDownloadItemActivated(const DownloadEntity*))));
...
#ifdef Q_OS_MAC
	VERIFY(connect(&DarwinSingleton::Instance(), SIGNAL(showPreferences()), this, SLOT(openPreferences())));
	VERIFY(connect(&DarwinSingleton::Instance(), SIGNAL(showAbout()), this, SLOT(about())));
#endif //Q_OS_MAC
...
void MainWindow::prepareToExit()
{
	...
	if ((m_downloadsForm != nullptr) && QSettings().value(app_settings::ClearDownloadsOnExit, app_settings::ClearDownloadsOnExit_Default).toBool())
	{
...
void MainWindow::openPreferences()
{
	...
	if (pPrefs.isNull())
	{
		...
		VERIFY(connect(pPrefs, SIGNAL(newPreferencesApply()), this, SLOT(resetFileSystemWatcher())));
	}
...
void MainWindow::onSearchItemActivated(RemoteVideoEntity* entity, const DownloadEntity* dEntity)
{
	...
	if (entity != nullptr)
	{
		if (m_player->state() == VideoPlayerWidget::InitialState || sender() == m_player)
		{
			if (dEntity != nullptr)
			{
				ui->descriptionWidget->setDescription(entity->m_videoInfo.strategyName, entity->m_videoInfo.description, dEntity->currentResolution());
			}
		}
	}
	else
...
void MainWindow::keyPressEvent(QKeyEvent* event)
{
	...
	else
	{
		...
		if ((event->key() == Qt::Key_Return || event->key() == Qt::Key_Enter) && ((event->modifiers() & Qt::AltModifier) != 0))
		{
...
SearchResultForm* MainWindow::searchForm()
{
	return m_searchResultFrom;
}

DownloadsForm* MainWindow::downloadsForm()
{
	return m_downloadsForm;
}

LibraryForm* MainWindow::libraryForm()
{
	return m_libraryForm;
}
...
void MainWindow::resetFileSystemWatcher()
{
	...
	VERIFY(connect(libraryWatcher, SIGNAL(directoryChanged(const QString&)), m_libraryForm->model(), SLOT(onVideoDirectoryChanged())));
...
{
	m_mainToolbar->switchTab(true);
}

void MainWindow::prevTab()
{
	m_mainToolbar->switchTab(false);
}

void MainWindow::openDownloadsTab(DownloadEntity* selEntity)
{
	m_mainToolbar->openDownloadsTab(selEntity);
}

void MainWindow::openLibraryTab(DownloadEntity* selEntity)
{
	m_mainToolbar->openLibraryTab(selEntity);
}

int MainWindow::getCurrTabIndex()
{
	return m_mainToolbar->getCurrTabIndex();
}
