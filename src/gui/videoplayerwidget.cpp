	...
	, m_videoWidget(new VideoWidget(this))
{
	...
	VERIFY(connect(this, SIGNAL(playDownloadEntityAsynchronously(const QPointer<DownloadEntity>&)),
				   this, SLOT(onPlayDownloadEntityAsynchronously(const QPointer<DownloadEntity>&)), Qt::QueuedConnection));
	VERIFY(connect(getDecoder(), SIGNAL(fileReleased(bool)), this, SLOT(updateViewOnVideoStop(bool))));
	VERIFY(connect(getDecoder(), SIGNAL(playingFinished()), this, SLOT(onPlayingFinished())));
	VERIFY(connect(getDecoder(), SIGNAL(fileProbablyNotFull()), this, SLOT(disableSeeking())));
	VERIFY(connect(getDecoder(), SIGNAL(downloadPendingStarted()), this, SLOT(showSpinner())));
	VERIFY(connect(getDecoder(), SIGNAL(downloadPendingFinished()), this, SLOT(hideSpinner())));
...
void VideoPlayerWidget::processPreviewEntity()
{
	...
	{
		VERIFY(connect(m_currentDownload, SIGNAL(fileCreated(QString)), this, SLOT(setVideoFilename(QString))));
		VERIFY(connect(m_currentDownload, SIGNAL(progressChanged(qint64, qint64)), this, SLOT(downloadingToPreview(qint64, qint64))));
		VERIFY(connect(m_currentDownload, SIGNAL(progressChanged(qint64, qint64)), getDecoder(), SLOT(limitPlayback(qint64, qint64))));
		VERIFY(connect(m_currentDownload, SIGNAL(finished()), getDecoder(), SLOT(resetLimitPlayback())));
		VERIFY(connect(m_currentDownload, SIGNAL(stateChanged(Downloadable::State, Downloadable::State)), this, SLOT(onDownloadStateChanged(Downloadable::State, Downloadable::State))));
	}
	...
	else if (m_currentDownload->state() == DownloadEntity::kDownloading || m_currentDownload->state() == DownloadEntity::kPaused)
	{
		VERIFY(connect(m_currentDownload, SIGNAL(progressChanged(qint64, qint64)), this, SLOT(downloadingToPreview(qint64, qint64))));
		VERIFY(connect(m_currentDownload, SIGNAL(progressChanged(qint64, qint64)), getDecoder(), SLOT(limitPlayback(qint64, qint64))));
		VERIFY(connect(m_currentDownload, SIGNAL(finished()), getDecoder(), SLOT(resetLimitPlayback())));
...
void VideoPlayerWidget::downloadingToPreview(qint64 bytesReceived, qint64 bytesTotal)
{
	...
	{
		qDebug() << "Opening file: " << m_currentFile << " at " << bytesReceived << " byte of " << bytesTotal << " bytes";
		VERIFY(disconnect((QObject*)sender(), SIGNAL(progressChanged(qint64, qint64)), this, SLOT(downloadingToPreview(qint64, qint64))));
...
void VideoPlayerWidget::setEntity(RemoteVideoEntity* entity)
{
	...
	if (state() == InitialState)
	{
		...
		if (entity != nullptr)
		{
			...
			m_descriptionPanel->setDescription(entity->m_videoInfo.strategyName, entity->m_videoInfo.description, entity->prefResolution());
		}
		else
		{
...
void VideoPlayerWidget::onPlayDownloadEntityAsynchronously(const QPointer<DownloadEntity>& e)
{
	...
	VERIFY(connect(m_currentDownload, SIGNAL(progressChanged(qint64, qint64)), m_progressBar, SLOT(displayDownloadProgress(qint64, qint64)), Qt::UniqueConnection));
...
void VideoPlayerWidget::setProgressbar(VideoProgressBar* progressbar)
{
	...
	VERIFY(connect(getDecoder(), SIGNAL(changedFramePosition(qint64, qint64)), m_progressBar, SLOT(displayPlayedProgress(qint64, qint64))));
...
void VideoPlayerWidget::playFile(const QString& fileName)
{
	...
	else
	{
		...
		setState(InitialState);
		m_controls->showPlaybutton(true);
		QMessageBox::information(this, tr(PROJECT_NAME), tr("File %1 cannot be played as it doesn't exist.").arg(fileName));
	}
}
...
void VideoPlayerWidget::updateViewOnVideoStop(bool showDefaultImage /* = false*/)
{
	...
	{
		m_descriptionPanel->setDescription(m_currentEntity->m_videoInfo.strategyName, m_currentEntity->m_videoInfo.description, m_currentDownload->currentResolution());
...
void  VideoPlayerWidget::setDefaultDisplay()
{
	setDisplay(new VideoWidget());
}
...
void VideoPlayerWidget::disableSeeking()
{
	Q_ASSERT(m_progressBar != nullptr);
}
