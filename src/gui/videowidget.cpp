	...
	m_lastMouseTime(0)
{
	...
	VERIFY(connect(&m_cursorTimer, SIGNAL(timeout()), this, SLOT(onCursorTimer())));
	VERIFY(connect(&m_imageCache, SIGNAL(getImageFinished(QImage)), this, SLOT(getImageFinished(QImage))));
	m_cursorTimer.start();
}
...
void VideoWidget::mousePressEvent(QMouseEvent* event)
{
	...
	if (event->button() == Qt::LeftButton && VideoPlayerWidgetInstance()->state() == VideoPlayerWidget::InitialState && pointInButton(event->pos())  && VideoPlayerWidgetInstance()->entity() != nullptr)
	{
...
void VideoWidget::mouseReleaseEvent(QMouseEvent* event)
{
	...
	{
		isFullScreen() ? fullScreen(false) : MainWindow::Instance()->dockWidget()->setVisibilityState(CustomDockWidget::FullScreen);
	}
}
...
void VideoWidget::mouseMoveEvent(QMouseEvent* event)
{
	...
	if (VideoPlayerWidgetInstance()->state() == VideoPlayerWidget::InitialState && VideoPlayerWidgetInstance()->entity() != nullptr)
	{
...
void VideoWidget::fullScreenProcess()
{
	...
	QWidget* videoControl = MainWindow::Instance()->videoControlWidget();
	videoControl->setParent(nullptr, Qt::FramelessWindowHint | Qt::Tool | Qt::WindowStaysOnTopHint); //Qt::WindowStaysOnTopHint
