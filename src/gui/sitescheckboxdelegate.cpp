...
void SitesCheckboxDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const
{
	...
	QRect bgr1Rect = QRect(opt.rect.left() + 1, opt.rect.top() + 1, checkBoxRect.width() + m_padding * 2, opt.rect.height() - 1);
	...
	if (!decorationRole.isNull() && decorationRole.canConvert(QVariant::Icon))
	{
		...
		opt.icon.paint(painter, opt.rect.x() + paintAtPos + 2, opt.rect.y() + 2, iconWidth - 4, opt.rect.height() - 4, Qt::AlignCenter);
	...
	QApplication::style()->drawItemText(painter, opt.rect.adjusted(paintAtPos, 0, 0, 0), Qt::AlignVCenter, opt.palette, true, opt.text);
}
