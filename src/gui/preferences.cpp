...
#endif
{
	...
	VERIFY(connect(updateVarButton, SIGNAL(pressed()), this, SLOT(updateDebugFilter())));

	VERIFY(connect(this, SIGNAL(appendDeveloperLog()), this, SLOT(onAppendDeveloperLog())));
	VERIFY(connect(this, SIGNAL(appendDeveloperMessage(QtMsgType, QString)), this, SLOT(onAppendDeveloperMessage(QtMsgType, QString))));

	setFilter(GET_SETTING(DebugFilterList));
#endif

#ifdef ALLOW_TRAFFIC_CONTROL
	ui->cbTrafficLimit->setChecked(settings.value(app_settings::IsTrafficLimited, app_settings::IsTrafficLimited_Default).toBool());
	ui->sbTrafficLimit->setValue(settings.value(app_settings::TrafficLimitKbs, app_settings::TrafficLimitKbs_Default).toInt());
	...
	VERIFY(connect(ui->listLang, SIGNAL(currentItemChanged(QListWidgetItem*, QListWidgetItem*)), this, SLOT(onCurrItemLangChanged(QListWidgetItem*))));
	VERIFY(connect(ui->tabWidget, SIGNAL(currentChanged(int)), this, SLOT(onCurrTabChanged(int))));
	...
	VERIFY(connect(ui->listSites, SIGNAL(itemPressed(QListWidgetItem*)), this, SLOT(toggleItem(QListWidgetItem*))));
}
...
bool Preferences::createSettingsDir(QString pathDir, const QString& strategyName)
{
	...
	if (!path.exists(pathDir) && !path.mkpath(pathDir))
	{
		...
		QMessageBox::critical(this, Tr::Tr(PROJECT_FULLNAME_TRANSLATION), tr("The folder \"%1\" cannot be created. Please choose another one.").arg(pathDir));
		...
		{
			ui->tabWidget->setCurrentIndex(0);
			QMessageBox::critical(this, Tr::Tr(PROJECT_FULLNAME_TRANSLATION), tr("The folder \"%1\" cannot be used, as there is no write access for %2. Please choose another one.").arg(pathDir, Tr::Tr(PROJECT_FULLNAME_TRANSLATION)));
			ui->lineEditOutputPath->setFocus();
		}
		return canProceed;
...
void Preferences::on_btnOutputPath_clicked()
{
	QString dir = QDir::toNativeSeparators(
					  QFileDialog::getExistingDirectory(
						  this,
						  utilities::Tr::Tr(DOWNLOAD_TO_LABEL),
...
void Preferences::onAppendDeveloperMessage(QtMsgType type, QString message)
{
	...
	{
		QString s = QDateTime::currentDateTimeUtc().toString("<font color=\"green\">[dd.MM.yy <b>hh:mm:ss</b>:zzz]</font>");
		QString filter;
...
QtMsgType Preferences::getTagId(const QString& tag)
{
	...
	QtMsgType result = (it != m_logFilter.end() && *it == tag) ? QtMsgType(it - m_logFilter.begin() + QtLoggerTagMsg) : QtRejectedLoggerTagMsg;
	m_logFilterLock.unlock();
