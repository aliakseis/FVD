	...
	m_searchPage(1)
{
	...
	VERIFY(connect(ui->searchResultView, SIGNAL(doubleClicked(QModelIndex)), this, SLOT(onViewDoubleClick(QModelIndex))));
	VERIFY(connect(ui->searchResultView, SIGNAL(customContextMenuRequested(QPoint)), this, SLOT(onSearchContextMenu(QPoint))));
	VERIFY(connect(ui->leSearch, SIGNAL(returnPressed()), this, SLOT(doSearch())));
	VERIFY(connect(ui->btnSearch, SIGNAL(clicked()), this, SLOT(doSearch())));
	...
	VERIFY(connect(m_model, SIGNAL(updateCurrentRow()), this, SLOT(onActivated())));

	VERIFY(connect(ui->searchResultView->verticalScrollBar(), SIGNAL(valueChanged(int)), this, SLOT(onVerticalScrollChanged(int))));
...
bool SearchResultForm::populateSitesComboHelper(const QStringList& strategiesNames, const QIcon& sectionIcon, const QString& sectionName)
{
    QSettings settings;
    QSet<QString> checkedSitesSet = settings.value(app_settings::CheckedSites, app_settings::CheckedSites_Default).toString().split(';', QString::SkipEmptyParts).toSet();
    QSet<QString> settingSitesSet = settings.value(app_settings::Sites, app_settings::Sites_Default).toString().split(';', QString::SkipEmptyParts).toSet();
	...
	if (!settingSitesSet.empty() && !strategiesNames.empty())
	{
		...
		Q_FOREACH(const QString & strategyName, strategiesNames)
		{
			if (settingSitesSet.contains(strategyName))
			{
				...
				ui->cbSites->addSite(icon, strategyName, strategyName);	// For now strategy name match combobox display text
			}
		}
	}
...
void SearchResultForm::onViewDoubleClick(const QModelIndex& index)
{
...
void SearchResultForm::doSearch()
{
	doSearch(ui->leSearch->text());
}
...
void SearchResultForm::onSearchFailed(const QString& error)
{
	qDebug() << "[SearchResultForm] " << error;
}
...
void SearchResultForm::onDownload(int row)
{
	...
	{
		QMessageBox msgBox(QMessageBox::Question, Tr::Tr(PROJECT_FULLNAME_TRANSLATION), tr("What videos would you like to download?"));
...
QWidget* SearchResultForm::manageWidget() const
{
...
void SearchResultForm::on_btnNext_clicked()
{
	doSearch(m_lastSearchString, ++m_searchPage);
}
...
void SearchResultForm::requestStartDownload(int row)
{
	m_model->item(row)->requestStartDownload();
}
