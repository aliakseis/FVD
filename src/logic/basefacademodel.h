...
{
public:
	BaseFacadeModel() {m_entities.reserve(128); entInd.reserve(128);}
...
protected:
	...
	void clear_impl() {m_entities.clear(); entInd.clear();}
	...
	{
		auto it = std::find_if(m_entities.begin(), m_entities.end(), [&pred](const StorageType * const ent) {return pred(ent);});
