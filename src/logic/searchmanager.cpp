...
void SearchManager::InitializeStrategies()
{
	if (m_scriptStrategies.isEmpty())
	{
		...
		foreach(const QString & str, listSites)
		{
			...
			VERIFY(QObject::connect(strategy, SIGNAL(searchResultsFound(const QList<SearchResult>&)), this, SLOT(onSearchResultsFound(const QList<SearchResult>&))));
		}
	}
}
...
void SearchManager::search(const QString& query, const QStringList& searchSites, int page)
{
	...
	Q_FOREACH(const QString & name, searchSites)
	{
        ...
        strategy->search(query,
            static_cast<strategies::SortOrder>(settings.value(SearchSortOrder, SearchSortOrder_Default).toInt()),
            settings.value(resultsOnPage, resultsOnPage_Default).toInt(),
            page);
	}
...
ScriptStrategy* SearchManager::scriptStrategy(const QString& name) const
{
	auto foundSearchStrategy =
		std::find_if(
			m_scriptStrategies.constBegin(), m_scriptStrategies.constEnd(),
...
void SearchManager::onDownloadStarted(const QString& url)
{
	emit signalDownloadStarted(url);
}
