...
template<typename F> class AdvisingClassFactory;
...
private:
	DownloadEntity(RemoteVideoEntity* parentVideoEntity = 0,
				   VisibilityState vState = visNorm,
...
public:
	...
	bool operator== (const DownloadEntity& download_entity) const {return m_parentVideoEntity->m_videoInfo.id == download_entity.m_parentVideoEntity->m_videoInfo.id;}
	...
	const QString& destinationPath() const {return m_downloader.isNull() ? m_destinationPath : m_downloader->destinationPath();}
	...
	QString description() const { Q_ASSERT(m_parentVideoEntity); return m_parentVideoEntity->m_videoInfo.description; }
...
private:
	...
	void decerializeSetVisibilityState(int value)
	{
		m_visibilityState = (VisibilityState) value;
	}
