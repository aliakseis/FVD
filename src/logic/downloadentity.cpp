...
DownloadEntity::DownloadEntity(RemoteVideoEntity* parentVideoEntity /* = 0*/,
...
void DownloadEntity::doSetup(const LinkInfo& linkInfo)
{
	...
	m_filepath = savePath + m_filepath + "(" + QString(linkInfo.resolution).remove(filenameEnemies) + ")." + linkInfo.extension;
...
bool DownloadEntity::doDownload()
{
	...
	{
		setState(kFailed);
		emit errorHappened(utilities::ErrorCode::eDOWLDOPENFILERR, "Cannot create " + global_functions::getSaveFolder(strategyName()));
		return false;
	}
...
{
	if (m_state != kDownloading) { return; }
	downloader()->Pause();
	setState(kPaused);
}
...
void DownloadEntity::doSetVisibilityState(VisibilityState value)
{
...
bool DownloadEntity::startDownloadWithHighestPriority()
{
	...
	return m_state == kDownloading || m_state == kFinished ||
		   doDownload();
}
...
DownloadEntity::DownloaderType* DownloadEntity::downloader()
{
	if (m_downloader.isNull())
	{
		...
		m_downloader->setDownloadNamePolicy(m_isFileAssigned ? DownloaderType::kReplaceFile : DownloaderType::kGenerateNewName);
