...
DownloadListModel::DownloadListModel(QObject* parent) :
	QAbstractListModel(parent), m_reportSM(true)
{
...
void DownloadListModel::onAppendEntities(const QList<DownloadEntity*>& list)
{
	if (!list.empty())
	{
		...
		Q_FOREACH(DownloadEntity * de, list)
		{
			...
			VERIFY(connect(de, SIGNAL(progressChanged(qint64, qint64)), this, SLOT(updateEntityRow())));
			VERIFY(connect(de, SIGNAL(speed(qint64)), this, SLOT(updateEntityRow())));
			VERIFY(connect(de, SIGNAL(stateChanged(Downloadable::State, Downloadable::State)), this, SLOT(entityStateChanged(Downloadable::State, Downloadable::State))));
			VERIFY(connect(de->getParent(), SIGNAL(linksExtracted()), this, SLOT(onLinkExtracted())));
		}
		endInsertRows();
...
bool DownloadListModel::removeRows(int row, int count, const QModelIndex& parent)
{
	...
	if (count > 0)
	{
		...
		for (const auto& ent : removedItems) { ent->disconnect(this); }
		endRemoveRows();
