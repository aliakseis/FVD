...
SearchResult::SearchResult()
	: published(nullDateTime), viewCount(0), duration(0)
{
}
...
DownloadEntity* RemoteVideoEntity::download(int resolutionId, VisibilityState visState)
{
	...
	auto it = std::find_if(m_downloads.begin(), m_downloads.end(), [&resolutionId](DownloadEntity * ent) { return (ent->currentResolutionId() == resolutionId); });
...
QList<DownloadEntity*> RemoteVideoEntity::allDownloadEntities() const
{
	return m_downloads;
}
...
void RemoteVideoEntity::setDownloads(const QObjectList& items)
{
	...
	for (auto item : items)
	{
		...
		if (entity->visibilityState() == visNorm)
		{
			...
			VERIFY(connect(entity, SIGNAL(stateChanged(Downloadable::State, Downloadable::State)), this, SIGNAL(signRVEUpdated())));
		}
	}
}
...
{
	return std::find_if(m_downloads.begin(), m_downloads.end(), &isDownloadEntityPersistable)
		   != m_downloads.end();
}
...
bool RemoteVideoEntity::restart(DownloadEntity* entity)
{
