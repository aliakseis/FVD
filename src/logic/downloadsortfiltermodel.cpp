	...
	Q_FOREACH(const QModelIndex & idx, indexes)
	{
		...
		if (idx.column() == 0)						// Column 0 contains data for Qt::UserRole - URL
		{
			...
			html += QString("<a href=\"%1\">%2</a>")
					.arg(data(idx, Qt::UserRole).toString(), data(index(idx.row(), DL_Title) , Qt::DisplayRole).toString());
		}
	}
...
bool DownloadSortFilterModel::dropMimeData(const QMimeData* data, Qt::DropAction action, int row, int column, const QModelIndex& parent)
{
	Q_UNUSED(parent)
	Q_UNUSED(action)
	Q_UNUSED(column)
...
Qt::DropActions DownloadSortFilterModel::supportedDropActions() const
{
	return Qt::MoveAction;
}

void DownloadSortFilterModel::setHeader(QHeaderView* headerView)
{
