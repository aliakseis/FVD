...
void LibraryModel::onAppendEntities(const QList<DownloadEntity*>& entitisList)
{
	...
	auto newEnd = std::remove_if(list.begin(), list.end(), [this](DownloadEntity * entity)
	{
	...
	if (!list.empty())
	{
		...
		{
			VERIFY(connect(ent, SIGNAL(stateChanged(Downloadable::State, Downloadable::State)), this, SLOT(entityStateChanged(Downloadable::State, Downloadable::State))));
		}
		MainWindow::Instance()->askForSavingModel();
	}
}
...
{
	QObjectList ents;
	iterateEntities([&ents](DownloadEntity * const ent)
	{
...
void LibraryModel::setEntities(const QObjectList& ents)
{
	...
	std::transform(ents.begin(), ents.end(), std::back_inserter(deList), [](QObject * const o)->DownloadEntity* {return qobject_cast<DownloadEntity*>(o);});
...
void LibraryModel::synchronize(bool mandatory)
{
	...
	if (!m_isLibraryRemoveWorkerRunning)
	{
		...
		connect(libraryRemoveWorker, &LibraryRemoveWorker::libraryFileDeleted, this, &LibraryModel::onLibraryFileDeleted);
		connect(libraryRemoveWorker, &LibraryRemoveWorker::destroyed, this, &LibraryModel::onLibraryRemoveWorkerDestroyed);
...
void LibraryModel::onLibraryUpdateTimer()
{
	synchronize(false);
}

void LibraryModel::onLibraryAddWorkerDestroyed()
{
	m_isLibraryAddWorkerRunning = false;
}

void LibraryModel::onLibraryRemoveWorkerDestroyed()
{
	m_isLibraryRemoveWorkerRunning = false;
}
...
void LibraryModel::onLibraryFileMissing(const QString& filePath)
{
	...
	iterateEntities([&entities, filePath](DownloadEntity * const entity)
	{
...
void LibraryModel::entityStateChanged(Downloadable::State newState, Downloadable::State oldState)
{
	...
	VERIFY(disconnect(ent, SIGNAL(stateChanged(Downloadable::State, Downloadable::State)), this, SLOT(entityStateChanged(Downloadable::State, Downloadable::State))));
...
void LibraryModel::onIdle()
{
	...
	Q_FOREACH(const QPointer<DownloadEntity>& missingEntity, missingFiles)
	{
		if (!missingEntity.isNull())
		{
			...
			if ((ent->visibilityState() == visLibOnly) || (ent->visibilityState() == visNorm && ent->state() == Downloadable::kFinished))
			{
...
void LibraryModel::scheduleOnIdle()
{
	QMetaObject::invokeMethod(this, "onIdle", Qt::QueuedConnection);
}

bool LibraryModel::hasWorkToDoOnIdle()
{
	return !m_allEntitiesFiles.empty() || !m_missingFiles.empty();
}
