...
{
	Q_UNUSED(dle)
	if ((oldState == Downloadable::kDownloading && state != Downloadable::kDownloading) || (Downloadable::kQueued == state))
	{
...
void DownloadManager::considerStartNextDownload()
{
	...
	const int maxDownloads = QSettings().value(app_settings::maximumNumberLoads, app_settings::maximumNumberLoads_Default).toInt();

	for (int i = 0; numActive < maxDownloads && i < numEntities(); ++i)
	{
		DownloadEntity* nextDl = item(i);
		if (nextDl->state() == DownloadEntity::kQueued
				&& !nextDl->directUrl().isEmpty()
				&& nextDl->download())
		{
			++numActive;
		}
...
void DownloadManager::clear_impl()
{
	BaseFacadeModel<DownloadEntity>::clear_impl();
}
...
void DownloadManager::resumeDownload(DownloadEntity* de)
{
	...
	if (numActiveDownloads < QSettings().value(app_settings::maximumNumberLoads, app_settings::maximumNumberLoads_Default).toInt())
	{
		de->resume();
	}
}
...
void DownloadManager::setSpeedLimit(DownloadEntity* de, int speedLimit)
{
	de->setSpeedLimit(speedLimit);
}

int DownloadManager::activeDownloadsCount() const
{
	int numActive = 0;
	iterateEntities([&numActive](const DownloadEntity * e) { if (DownloadEntity::kDownloading == e->state()) { ++numActive; } });
	return numActive;
}
