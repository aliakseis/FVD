...
SearchListModel::SearchListModel(QObject* parent) : QAbstractListModel(parent), m_headerView(nullptr)
{
    VERIFY(connect(&SearchManager::Instance(), SIGNAL(searchResultFound(QList<RemoteVideoEntity*>)),
                   SLOT(onAppendEntities(QList<RemoteVideoEntity*>))));
}
...
QVariant SearchListModel::data(const QModelIndex& index, int role) const
{
    ...
    else if (role == Qt::ToolTipRole)
    {
        ...
        if (auto* view = qobject_cast<QAbstractItemView*>(this->QObject::parent()))
        {
            ...
            {
                QString text =
                    (index.column() == SR_Title) ? entity->m_videoInfo.videoTitle : entity->m_videoInfo.description;
...
void SearchListModel::setHeaderView(QHeaderView* headerView) { m_headerView = headerView; }
...
void SearchListModel::onAppendEntities(const QList<RemoteVideoEntity*>& list)
{
    if (!list.empty())
    {
        ...
        {
            VERIFY(connect(rve, SIGNAL(signRVEUpdated()), SLOT(updateEntityRow())));
            VERIFY(connect(rve, SIGNAL(signRVEProgressUpdated()), SLOT(updateProgressEntityRow())));
        }
        endInsertRows();
    }
}
